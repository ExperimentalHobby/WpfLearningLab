...
8. **`WpfLearningLab.slnx` は複数ブランチが同じ位置（`</Solution>` 直前）に `<Folder>` を追記するためマージコンフリクトが頻発する**: **`merge=union` を設定してはならない。** union は行単位で「衝突箇所の両方の行を残す」戦略でありXMLの入れ子構造を理解しないため、複数ブランチが追加した同一内容の閉じタグ行 `  </Folder>` が1行に潰され、閉じタグ欠落でslnxがXMLとして壊れ**ソリューション全体がビルド不能になる障害が実際に発生した**（一度この設定を入れて撤回済み）。コンフリクトは既定の3-wayマージで手動解決し、**「両方の`<Folder>`ブロックを閉じタグごと残す」**形に直すこと。解決後は必ず `dotnet build WpfLearningLab.slnx` が通ること（＝XMLとして妥当であること）を確認する。またマージ解決中に `WpfLearningLab.slnx.bak` / `.orig` やマージツール由来の一時ファイル（`WpfLearningLab_BACKUP_*.slnx` 等）が生成されることがあるが、これらは Git 管理対象外なのでマージ確定後に削除してよい
