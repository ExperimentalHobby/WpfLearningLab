...
8. **`WpfLearningLab.slnx` は複数ブランチが同じ位置（`</Solution>` 直前）に `<Folder>` を追記するためマージコンフリクトが頻発する**: `.gitattributes` に `WpfLearningLab.slnx merge=union` を設定し、追記どうしの衝突は自動マージされるようにしている。それでも `<Folder>` の削除・パス変更（リネーム等）が絡む場合は union では正しくマージされないことがあるため、コンフリクト時は差分を目視確認すること。またマージ解決中に `WpfLearningLab.slnx.bak` / `.orig` やマージツール由来の一時ファイル（`WpfLearningLab_BACKUP_*.slnx` 等）が生成されることがあるが、これらは Git 管理対象外なのでマージ確定後に削除してよい
