...
public class CsvParserTests
{
	...
	/// <summary>
	/// パス条件: ダブルクォートで囲まれたフィールド内に改行を含む値が、複数の物理行(File.ReadLines等で
	/// 分割済みの状態)にまたがっていても、改行を含む1つの値として正しく結合されること
	/// (クラスのdocコメントは改行対応を謳うが、実装が追いついていなかった不具合の回帰テスト)。
	/// </summary>
	[Fact]
	public void Parse_フィールド内の改行が複数行にまたがっていても正しく結合される()
	{
		var lines = new[]
		{
			"名前,備考",
	...
	/// <summary>
	/// パス条件: ヘッダーに重複した列名がある場合、2件目以降に連番が付与され、
	/// 両方の列の値が(後勝ちで消えることなく)正しく保持されること。
	/// </summary>
	[Fact]
	public void Parse_ヘッダーに重複がある場合は連番で一意化される()
	{
		var lines = new[]
		{
			"名前,名前",
		...
		Assert.Equal(new[] { "名前", "名前 (2)" }, table.Headers);
		Assert.Equal("山田", table.Rows[0]["名前"]);
		Assert.Equal("太郎", table.Rows[0]["名前 (2)"]);
	}
}
